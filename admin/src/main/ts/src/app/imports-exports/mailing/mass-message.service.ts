...
export class MassMessageService {
    ...
    static async getColumnsMapping(importInfos): Promise<any> {
        return this.buildPostFormData(importInfos, 'wizard/massmessaging/column/mapping');
    }

    static async populateImportedInfos(mappings, requiredFields): Promise<any> {
        let response;
        try {
            response = await http.post('/directory/wizard/massmessaging/validation/populate',
    ...
    static async sendEmail(importedData): Promise<any> {
        ...
        let response;
                try {
                    response = await http.post('/directory/wizard/massmessaging',data,
                     {headers : { 'Content-Type': 'application/json' }});
                     return response.data;
                } catch (error) {
