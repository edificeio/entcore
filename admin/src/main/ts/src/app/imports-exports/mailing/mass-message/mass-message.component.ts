...
export class MassMessageComponent extends OdeComponent implements OnInit, OnDestroy {
    ...
    messagesSent = false;
    sendFailed = false;
    showFailedLoginsPopup: boolean = false;
    structureId: string;
    show = false;
    ...
    */
    ...
    async sendAsmPublipostage(): Promise<void> {
    
        if(this.inputValue == null) {
            this.globalError.message = "Can't send message without subject";
        } else {
                this.importedData.template = this.htmlTemplate;
                this.importedData.messageSubject = this.inputValue;
            try {
                const data = await MassMessageService.sendEmail(this.importedData);
                this.numberMessagesSent = data.loginsSucceeded;
                this.loginsFailed = data.remainingLogins;
                this.messagesSent = true;
                this.globalError.message = null;
                this.changeDetector.detectChanges();
            } catch (error) {
    ...
    async getSenderName(): Promise<void> {
        try {
          const data = await MassMessageService.getSenderName();
