...
function isNoDirectionChangeResponse(response: CreateCommunicationResponseWithNoChange): boolean {
...
export class CommunicationRulesService {
    ...
    public setGroups(groups: GroupModel[]) {
        forkJoin(
                forkJoin(...groups.map(group => this.getSendingCommunicationRulesOfGroup(group))),
                forkJoin(...groups.map(group => this.getReceivingCommunicationRulesOfGroup(group)))
                .pipe(
            ...
            .pipe(map(arr => ({sending: arr[0], receiving: arr[1]})))
                ...
                tap((internalCommunicationRule) => {
                    if (this.currentRules) {
                        ...
                        if (groupsInCommunicationRules.length > 0) {
                            groupsInCommunicationRules.forEach(sgroup => sgroup.internalCommunicationRule = internalCommunicationRule);
    ...
    public createCommunication(sender: GroupModel, receiver: GroupModel, positionOfUserGroup: Column): Observable<CreateCommunicationResponse> {
        return this.http.post<CreateCommunicationResponse>(`/communication/v2/group/${sender.id}/communique/${receiver.id}`, {})
            .pipe(
                tap((createCommunicationResponse: CreateCommunicationResponse) => {
                    if (!isNoDirectionChangeResponse(createCommunicationResponse as CreateCommunicationResponseWithNoChange)) {
                    ...
                    if (this.currentRules) {
                        if (positionOfUserGroup === 'sending') {
                            ...
                            if (!!emptyCommunicationRule) {
                                ...
                                this.currentRules.receiving.push({sender, receivers: [receiver]});
                            }
                        }
                        this.$rulesSubject.next(this.clone(this.currentRules));
                    }
