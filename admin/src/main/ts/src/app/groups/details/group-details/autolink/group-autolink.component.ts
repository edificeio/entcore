...
import {
  GroupsService,
  GroupUpdatePayload,
} from "src/app/groups/groups.service";
...
export class GroupAutolinkComponent extends OdeComponent {
  ...
  private initForm(): void {
    ...
    if (
      this.group.autolinkTargetStructs &&
      this.group.autolinkTargetStructs.length > 0
    ) {
      this.form.subStructuresRadio = "manual";
      this.form.subStructuresIds = this.group.autolinkTargetStructs;
      this.lightboxSubStructureIds = Object.assign(
        [],
    ...
    if (
      this.group.autolinkUsersFromGroups &&
      this.group.autolinkUsersFromGroups.length > 0
    ) {
      ["Teacher", "Personnel", "Student", "Relative", "AdminLocal"].forEach(
        (profile) => {
  ...
  public onAutolinkSubmit() {
    ...
    if (this.form.profile === "Teacher") {
      if (this.form.teacherSubSectionRadio === "HeadTeacher") {
        ...
        groupUpdatePayload.autolinkUsersFromGroups =
          groupUpdatePayload.autolinkUsersFromGroups.concat(
            this.form.selectedDisciplines
          );
    ...
    this.groupsService.update(this.group.id, groupUpdatePayload).subscribe(
      () => {
      ...
      return {
        ...
        children:
          child.children && child.children.length > 0
            ? child.children.map(myMap)
            : [],
        check:
          !!this.lightboxSubStructureIds &&
          !!this.lightboxSubStructureIds.find((subId) => subId === child.id),
      };
    };
  ...
  public addOrRemoveChild(child: StructureListItem): void {
    const index = this.lightboxSubStructureIds.findIndex(
      (subId) => subId === child.id
    );
    if (index === -1) {
      this.lightboxSubStructureIds.push(child.id);
      this.checkAllChildren(child.children);
    } else {
      this.lightboxSubStructureIds = this.lightboxSubStructureIds
        .slice(0, index)
        .concat(
          this.lightboxSubStructureIds.slice(
            index + 1,
            this.lightboxSubStructureIds.length
          )
        );
  ...
  public unselectDiscipline(item: string): void {
    if (this.showActions) {
      this.form.selectedDisciplines.splice(
        this.form.selectedDisciplines.indexOf(item),
        1
      );
    }
  }
  ...
  public unselectFunction(item: string): void {
    if (this.showActions) {
      this.form.selectedFunctions.splice(
        this.form.selectedFunctions.indexOf(item),
        1
      );
    }
  }
  ...
  private checkAllChildren(children: Array<StructureListItem>) {
    children.forEach((child) => {
      child.check = true;
      if (
        this.lightboxSubStructureIds.findIndex(
          (subId) => subId === child.id
        ) === -1
      ) {
        this.lightboxSubStructureIds.push(child.id);
