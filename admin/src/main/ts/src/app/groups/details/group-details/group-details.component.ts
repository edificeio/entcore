...
} from "src/app/core/store/models/group.model";
  ...
  implements OnInit, OnDestroy
{
  ...
  public $confirmationClicked: Subject<"confirm" | "cancel"> = new Subject<
    "confirm" | "cancel"
  >();

  public $deleteButtonClicked: Subject<GroupModel> = new Subject();
  public deleteConfirmationDisplayed = false;
  public $deleteConfirmationClicked: Subject<"confirm" | "cancel"> =
    new Subject<"confirm" | "cancel">();

  public $renameButtonClicked: Subject<{}> = new Subject();
  public renameLightboxDisplayed = false;
  public renameConfirmationClicked: Subject<"confirm" | "cancel"> = new Subject<
    "confirm" | "cancel"
  >();
  ...
  ngOnInit() {
    ...
    const rulesChangesObserver = merge(
      ...
      this.$toggleCommunicationRuleClicked.pipe(
      ...
      map(
        (data: GroupIdAndInternalCommunicationRule) =>
          data.internalCommunicationRule
      ),
      tap((rule) => (this.internalCommunicationRule = rule))
    );

    const groupChangesObserver = this.route.params.pipe(
    ...
    this.subscriptions.add(
      merge(rulesChangesObserver, groupChangesObserver).subscribe(() =>
    ...
    this.subscriptions.add(
      this.$deleteButtonClicked
        .pipe(mergeMap((group: GroupModel) => this.deleteGroup(group)))
        .subscribe()
    );

    this.subscriptions.add(
      this.$renameButtonClicked
        .pipe(mergeMap(() => this.renameGroup()))
        .subscribe()
    );
        ...
        new Promise<void>((resolve, reject) => {
          this.groupsService
            ...
            .subscribe((data: Array<GroupModel>) => {
              const disciplineGroups = data.filter(
                (group) =>
                  group.labels && group.labels.includes("DisciplineGroup")
              );
              if (disciplineGroups) {
                this.autolinkDisciplineOptions = disciplineGroups.map(
                  (d) => d.filter
                );
                // remove duplicates values
                this.autolinkDisciplineOptions = Array.from(
                  new Set(this.autolinkDisciplineOptions)
                );
                this.autolinkDisciplineOptions.sort();
              }

              const funcGroups = data.filter(
                (group) => group.labels && group.labels.includes("FuncGroup")
              );
              if (funcGroups) {
                this.autolinkFunctionOptions = funcGroups.map((f) => f.filter);
                // remove duplicates values
                this.autolinkFunctionOptions = Array.from(
                  new Set(this.autolinkFunctionOptions)
                );
  ...
  toggleCommunicationBetweenMembers(
    group: GroupModel
  ): Observable<GroupIdAndInternalCommunicationRule> {
    this.confirmationDisplayed = true;
    return this.$confirmationClicked.asObservable().pipe(
      first(),
      tap(() => (this.confirmationDisplayed = false)),
      filter((choice) => choice === "confirm"),
      mergeMap(() =>
        this.communicationRulesService.toggleInternalCommunicationRule({
          id: group.id,
  ...
  public deleteGroup(group: GroupModel): Observable<void> {
    ...
    return this.$deleteConfirmationClicked.asObservable().pipe(
      first(),
  ...
  public openGroupCommunication(group: GroupModel) {
    this.router.navigate([group.id, "communication"], {
  ...
  public renameGroup(): Observable<void> {
    ...
    return this.renameConfirmationClicked.asObservable().pipe(
      first(),
      ...
      tap(
        () => {
          this.notifyService.success(
            "group.rename.notify.success.content",
            "group.rename.notify.success.title"
          );
        },
        (error: HttpErrorResponse) => {
          this.notifyService.error(
            "group.rename.notify.error.content",
