      ...
      .check(status.is(200),
        bodyString.find.transformOption(_.map{res =>
          ...
          json.asScala.foldLeft[List[String]](List("","")){(acc, c) =>
            val app = c.asInstanceOf[JSONObject]
            app.get("name").asInstanceOf[String] match {
              case "Enseignants du groupe Ecole primaire Emile Zola." | "Enseignants du groupe E.Zola renamed." | "Enseignants du groupe Emile Zola." => List(app.get("id").asInstanceOf[String], acc.last)
              case "Élèves du groupe Ecole primaire Emile Zola." | "Élèves du groupe E.Zola renamed." | "Élèves du groupe Emile Zola." => List(acc.head, app.get("id").asInstanceOf[String])
              case _ => acc
            }
          }
