...
try {
  ...
  fileToUpload = open(
    `${dataRootPath}/data/workspace/random_text_file.txt`,
    "b"
  );
}
...
export function setup() {
  ...
  describe("[Workspace-Init] Initialize data", () => {
    ...
    const groupIds = [teacherRole1.id, teacherRole2.id];
...
function checkShareKo(res, checkName) {
  ...
  checks[`${checkName} - insufficient rights error returned`] = (r) => {
    try {
      return (
        JSON.parse(r.body).error === "insufficient.rights.to.modify.shares"
      );
    } catch (e) {
      return true;
    }
...
function checkPresentShares(
  res,
  sharesType,
  expectedSharesToBePresent,
  checkName
) {
  const checks = {};
  checks[checkName] = (r) => {
    return expectedSharesToBePresent.every((expectedShare) =>
      r[sharesType].visibles
        .map((actualShare) => actualShare.id)
        .includes(expectedShare)
    );
  };
  const ok = check(res, checks);
  if (!ok) {
...
function checkHasCorrectShares(
  res,
  sharesType,
  entityId,
  expectedShares,
...
function checkAbsentShares(
  res,
  sharesType,
  expectedSharesToBeAbsent,
  checkName
) {
  const checks = {};
  checks[checkName] = (r) => {
    return expectedSharesToBeAbsent.every(
      (expectedShare) =>
        !r[sharesType].visibles
          .map((actualShare) => actualShare.id)
          .includes(expectedShare)
...
function testEmptyShares(data) {
  ...
  describe("[Workspace] Test a user can share to no one", () => {
    ...
    const teacher13 = getRandomUserWithProfile(users1, "Teacher", [
      teacher1,
      teacher12,
    ]);
    ...
    describe("Erase all the shares", () => {
      ...
      checkShareOk(
        res,
        "200 while sharing to no one a file that was shared before"
      );
    });
  });
}
...
function testForbiddenShares(data) {
  ...
  describe("[Workspace] Test forbidden shares via profile groups in two different schools", () => {
    ...
    checkPresentShares(
      ...
      "groups",
      [parentRole2.id],
      "parents group of school 2 can see"
    );
    checkPresentShares(
      res,
      "users",
      [student1.id],
      "student of group of school 1 can see"
    );
    checkPresentShares(
      res,
      "users",
      [parent1.id],
      "parent of group of school 1 can see"
    );
    checkPresentShares(
      res,
      "users",
      [teacher12.id],
      "teacher of group of school 1 can see"
    );
    checkPresentShares(
      res,
      "users",
    ...
    checkShareOk(
      res,
      "200 while trying to remove shares of a user he/she cannot see"
    );
    res = getShares(fileId);
    checkPresentShares(
      res,
      "groups",
      [parentRole2.id],
      "parents group of school 2 still appears in shares after forbidden removal"
    );
    checkPresentShares(
      res,
      "users",
      [parent1.id],
      "parent of group of school 1 still appears in shares after forbidden removal"
    );
    checkPresentShares(
      res,
      "users",
      [teacher12.id],
      "teacher of group of school 1 still appears in shares after forbidden removal"
    );
    checkPresentShares(
      res,
      "users",
    ...
    checkShareKo(
      res,
      "400 while trying to modify shares of a user he/she cannot see"
    );
    res = getShares(fileId);
    checkPresentShares(
      res,
      "groups",
      [parentRole2.id],
      "parents group of school 2 still appears in shares after forbidden modification"
    );
    checkPresentShares(
      res,
      "users",
      [parent1.id],
      "parent of group of school 1 still appears in shares after forbidden modification"
    );
    checkPresentShares(
      res,
      "users",
      [teacher12.id],
      "teacher of group of school 1 still appears in shares after forbidden modification"
    );
    checkHasCorrectShares(
      res,
      "users",
      teacher13.id,
      WS_MANAGER_SHARE,
      "teacher of group of school 1 still has manager rights after forbidden modification"
    );
    ...
    checkShareOk(
      res,
      "400 while trying to modify and delete shares of a user he/she cannot see"
    );
    res = getShares(fileId);
    checkPresentShares(
      res,
      "groups",
      [parentRole2.id],
      "parents group of school 2 still appears in shares after forbidden modification and deletion"
    );
    checkPresentShares(
      res,
      "users",
      [parent1.id],
      "parent of group of school 1 still appears in shares after forbidden modification and deletion"
    );
    checkPresentShares(
      res,
      "users",
      [teacher12.id],
      "teacher of group of school 1 still appears in shares after forbidden modification and deletion"
    );
    checkHasCorrectShares(
      res,
      "users",
      teacher13.id,
      WS_MANAGER_SHARE,
      "teacher of group of school 1 still has manager rights after forbidden modification and deletion"
    );
  });
}
...
function testSharesViaProfileGroupInDifferentSchools(data) {
  ...
  describe("[Workspace] Test shares via profile groups in two different schools", () => {
    ...
    checkShareOk(
      res,
      "teacher of school 1 shares to parents group of school 2 and parent of school 1"
    );
    res = getShares(fileId);
    checkPresentShares(
      res,
      "users",
      [parent1.id],
      "parent of school 1 appears in shares"
    );
    checkPresentShares(
      res,
      "groups",
      [parentRole2.id],
      "parents group of school 2 appears in shares"
    );
    ...
    checkPresentShares(
      ...
      "groups",
      [studentRole2.id],
      "student group of school 2 appears in shares"
    );
    checkPresentShares(
      res,
      "users",
      [parent1.id],
      "parent of school 1 still appears in shares after parent of school 2 shares"
    );
    ...
    checkShareKo(
      res,
      "other teacher of school 1 tries to modify shares of student of school 2"
    );
    ...
    checkShareOk(
      res,
      "parent of school 2 tries to modify shares of student of school 2"
    );
    ...
    checkPresentShares(
      ...
      "users",
      [parent1.id],
      "parent of school 1 still appears in shares after shares from school 2"
    );
    ...
    checkShareOk(
      res,
      "parent of school 2 removes shares to student of school 2"
    );
    res = getShares(fileId);
    checkPresentShares(
      res,
    ...
    checkPresentShares(
      ...
      "groups",
      [parentRole2.id],
      "parents group of school 2 appears in shares after shares from school 2"
    );
    checkAbsentShares(
      res,
      "groups",
      [studentRole2.id],
      "student group of school 2 does not appear in shares after shares from school 2"
    );
...
function testSharesViaBroadcastGroupInDifferentSchools(data) {
  ...
  describe("[Workspace] Test shares to broadcast group in two different schools", () => {
    ...
    const users1 = getUsersOfSchool(structure1);
    const users2 = getUsersOfSchool(structure2);
    const broadcastGroup = getBroadcastGroup(
      broadcastGroupNameChapeau,
      chapeau
    );
    const broadcastGroupSchool1 = getBroadcastGroup(
      broadcastGroupName,
      structure1
    );
    const teachers1 = users1
      .filter((u) => u.type === "Teacher")
      .filter((t) => t.classes && t.classes.length > 0);
    const teachers2 = users2
      .filter((u) => u.type === "Teacher")
      .filter((t) => t.classes && t.classes.length > 0);
    ...
    const classUsers1 = students1.filter(
      (u) => u.classes.map((c) => c.id).indexOf(classId1) >= 0
    );
    const classUsers2 = students2.filter(
      (u) => u.classes.map((c) => c.id).indexOf(classId2) >= 0
    );
    ...
    checkPresentShares(
      ...
      "groups",
      [broadcastGroup.id],
      "chapeau broadcast group appears in shares"
    );
    ...
    checkPresentShares(
      ...
      "users",
      [firstStudent2.id],
      "student of school 2 appears in shares"
    );
    ...
    checkShareKo(
      res,
      "student of school 2 shares to broadcast group of school 1"
    );
  });
}
...
function testSharesViaBroadcastGroupInSameSchool(data) {
  ...
  describe("[Workspace] Test shares to broadcast group in the same school", () => {
    ...
    const classUsers = students.filter(
      (u) => u.classes.map((c) => c.id).indexOf(classId) >= 0
    );
    const firstStudent = getRandomUser(classUsers);
    const secondStudent = getRandomUser(classUsers, [firstStudent]);
    const thirdStudent = getRandomUser(classUsers, [
      firstStudent,
      secondStudent,
    ]);
    ...
    checkPresentShares(
      ...
      "users",
      [firstStudent.id],
      "first student appears in shares"
    );
    // First student tries to share it to the list -> not shared because not visible
    let firstStudentSession = <Session>(
      authenticateWeb(firstStudent.login, "password")
    );
    ...
    checkAbsentShares(
      ...
      "groups",
      [broadcastGroup.id],
      "broadcast group does not appear in shares"
    );
    ...
    checkPresentShares(
      ...
      "users",
      [secondStudent.id],
      "second student appears in shares"
    );
    ...
    checkPresentShares(
      ...
      "groups",
      [broadcastGroup.id],
      "broadcast group appears in shares"
    );
    ...
    checkPresentShares(
      ...
      "users",
      [thirdStudent.id],
      "third student appears in shares"
    );
...
function testSharesViaFavouritesWithUsersWhoBecameInvisible(data) {
  ...
  describe("[Workspace] Test shares via favourites and invisible users", () => {
    ...
    res = addCommunicationBetweenGroups(
      teacherProfileGroup.id,
      guestProfileGroup.id
    );
    ...
    checkPresentShares(
      ...
      "users",
      [newTeacher.id],
      "user of sharebookmark who is still visible appears in shares"
    );
    checkAbsentShares(
      res,
      "users",
      [newGuest.id],
      "user of sharebookmark who is now invisible does not appear in shares"
    );
  });
}
