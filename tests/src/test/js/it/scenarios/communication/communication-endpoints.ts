...
import {checkEquals, checkFalse, checkTrue, checkUsersCanCommunicate, checkNotEquals, checkGte} from "./_utils.ts";
...
const enabledScenarios = __ENV.ACTIVATED_SCENARIOS ? __ENV.ACTIVATED_SCENARIOS.split(",") : availableScenarioNames;
...
export function testRemoveLink(data: InitData) {
  group('[Endpoints] removeLink', () => {
    ...
    checkEquals(res, `${testName} removeLink returns 200`, 200, res.status);
...
/***************************************************************************************************
 * addLinksWithUsers
 * POST /group/:groupId?direction=BOTH
 ***************************************************************************************************/
export function testAddLinksWithUsers(data: InitData) {
  group('[Endpoints] addLinksWithUsers', () => {
    ...
    checkEquals(res, `${testName} addLinksWithUsers returns 200`, 200, res.status);
...
/***************************************************************************************************
 * removeLinksWithUsers
 * DELETE /group/:groupId?direction=BOTH
 ***************************************************************************************************/
export function testRemoveLinksWithUsers(data: InitData) {
  group('[Endpoints] removeLinksWithUsers', () => {
    ...
    checkEquals(res, `${testName} removeLinksWithUsers returns 200`, 200, res.status);
...
export function testCommuniqueWith(data: InitData) {
  group('[Endpoints] communiqueWith - no link returns empty communiqueWith', () => {
    ...
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkEquals(res, `${testName} response has id field matching the group`, group1.id, body.id);
    checkTrue(res, `${testName} response contains communiqueWith array`, Array.isArray(body.communiqueWith));
    checkEquals(res, `${testName} communiqueWith is empty when no link exists`, 0, body.communiqueWith.length);
  ...
  group('[Endpoints] communiqueWith - single link', () => {
    ...
    const body = JSON.parse(<string>res.body);
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkTrue(res, `${testName} response has id field`, typeof body.id === "string" && body.id === group1.id);
    checkTrue(res, `${testName} response contains communiqueWith array`, Array.isArray(body.communiqueWith));
    checkEquals(res, `${testName} communiqueWith has exactly 1 entry`, 1, body.communiqueWith.length);
    checkTrue(res, `${testName} communiqueWith entry has id field`, typeof body.communiqueWith[0].id === "string");
    checkEquals(res, `${testName} communiqueWith contains target group`, group2.id, body.communiqueWith[0].id);
    ...
    checkEquals(resReverse, `${testName} reverse returns 200`, 200, resReverse.status);
    checkFalse(resReverse, `${testName} reverse communiqueWith does NOT contain source group`, bodyReverse.communiqueWith.some((g: any) => g.id === group1.id));
  ...
  group('[Endpoints] communiqueWith - multiple links', () => {
    ...
    const body = JSON.parse(<string>res.body);
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkEquals(res, `${testName} communiqueWith has 2 entries`, 2, body.communiqueWith.length);
    checkTrue(res, `${testName} communiqueWith contains group2`, body.communiqueWith.some((g: any) => g.id === group2.id));
    checkTrue(res, `${testName} communiqueWith contains group3`, body.communiqueWith.some((g: any) => g.id === group3.id));
  ...
  group('[Endpoints] communiqueWith - after removing link', () => {
    ...
    checkTrue(resBefore, `${testName} before removal contains target group`, bodyBefore.communiqueWith.some((g: any) => g.id === group2.id));
    ...
    checkEquals(resAfter, `${testName} after removal returns 200`, 200, resAfter.status);
    checkFalse(resAfter, `${testName} after removal communiqueWith does NOT contain removed group`, bodyAfter.communiqueWith.some((g: any) => g.id === group2.id));
...
export function testGetOutgoingRelations(data: InitData) {
  group('[Endpoints] getOutgoingRelations - empty when no link exists', () => {
    ...
    const body = JSON.parse(<string>res.body);
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkTrue(res, `${testName} response is an array`, Array.isArray(body));
    checkEquals(res, `${testName} outgoing is empty`, 0, body.length);
  ...
  group('[Endpoints] getOutgoingRelations - contains target after adding link', () => {
    ...
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkTrue(res, `${testName} response is an array`, Array.isArray(body));
    checkTrue(res, `${testName} outgoing contains target group`, body.some((g: any) => g.id === group2.id));
    checkTrue(res, `${testName} entries have id field`, body.every((g: any) => typeof g.id === "string"));
    ...
    checkFalse(resTarget, `${testName} target outgoing does NOT contain source`, bodyTarget.some((g: any) => g.id === group1.id));
  ...
  group('[Endpoints] getOutgoingRelations - multiple outgoing links', () => {
    ...
    const body = JSON.parse(<string>res.body);
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkTrue(res, `${testName} outgoing contains group2`, body.some((g: any) => g.id === group2.id));
    checkTrue(res, `${testName} outgoing contains group3`, body.some((g: any) => g.id === group3.id));
  ...
  group('[Endpoints] getOutgoingRelations - disappears after removing link', () => {
    ...
    checkTrue(resBefore, `${testName} before removal contains target`, JSON.parse(<string>resBefore.body).some((g: any) => g.id === group2.id));
    ...
    const bodyAfter = JSON.parse(<string>resAfter.body);
    checkEquals(resAfter, `${testName} after removal returns 200`, 200, resAfter.status);
    checkFalse(resAfter, `${testName} after removal does NOT contain removed group`, bodyAfter.some((g: any) => g.id === group2.id));
...
export function testGetIncomingRelations(data: InitData) {
  group('[Endpoints] getIncomingRelations - empty when no link exists', () => {
    ...
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkTrue(res, `${testName} response is an array`, Array.isArray(body));
    checkEquals(res, `${testName} incoming is empty`, 0, body.length);
  ...
  group('[Endpoints] getIncomingRelations - contains source after adding link', () => {
    ...
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkTrue(res, `${testName} response is an array`, Array.isArray(body));
    checkTrue(res, `${testName} incoming contains source group`, body.some((g: any) => g.id === group1.id));
    checkTrue(res, `${testName} entries have id field`, body.every((g: any) => typeof g.id === "string"));
    ...
    checkFalse(resSource, `${testName} source incoming does NOT contain target`, bodySource.some((g: any) => g.id === group2.id));
  ...
  group('[Endpoints] getIncomingRelations - multiple incoming links', () => {
    ...
    const body = JSON.parse(<string>res.body);
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkTrue(res, `${testName} incoming contains group1`, body.some((g: any) => g.id === group1.id));
    checkTrue(res, `${testName} incoming contains group2`, body.some((g: any) => g.id === group2.id));
  ...
  group('[Endpoints] getIncomingRelations - disappears after removing link', () => {
    ...
    const body = JSON.parse(<string>res.body);
    checkEquals(res, `${testName} after removal returns 200`, 200, res.status);
    checkFalse(res, `${testName} after removal does NOT contain removed group`, body.some((g: any) => g.id === group1.id));
...
/***************************************************************************************************
 * addLinkBetweenRelativeAndStudent
 * POST /relative/:groupId?direction=BOTH
 ***************************************************************************************************/
export function testAddLinkBetweenRelativeAndStudent(data: InitData) {
  group('[Endpoints] addLinkBetweenRelativeAndStudent - direction BOTH', () => {
    ...
    const body = JSON.parse(<string>res.body);
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkNotEquals(res, `${testName} response contains number field`, undefined, body.number);
    checkEquals(res, `${testName} number is a number`, "number", typeof body.number);
    checkGte(res, `${testName} number is >= 0`, 0, body.number);
  });
...
/***************************************************************************************************
 * removeLinkBetweenRelativeAndStudent
 * DELETE /relative/:groupId?direction=BOTH
 ***************************************************************************************************/
export function testRemoveLinkBetweenRelativeAndStudent(data: InitData) {
  ...
  group('[Endpoints] removeLinkBetweenRelativeAndStudent - remove INCOMING only', () => {
    ...
    const body = JSON.parse(<string>res.body);
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkTrue(res, `${testName} number is a number`, typeof body.number === "number");
  });
}
...
export function testVisibleUsers(data: InitData) {
  ...
  group('[Endpoints] visibleUsers - contains user in same group with comm enabled', () => {
    ...
    const body = JSON.parse(<string>res.body);
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkTrue(res, `${testName} visible list contains the other teacher`, body.some((v: any) => v.id === teacher2.id));
  ...
  group('[Endpoints] visibleUsers - does NOT contain user without comm link', () => {
    ...
    const body = JSON.parse(<string>res.body);
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkFalse(res, `${testName} visible list does NOT contain isolated user`, body.some((v: any) => v.id === teacher2.id));
  });
}

/***************************************************************************************************
 * visibleGroupContains
...
export function testVisibleGroupContains(data: InitData) {
  group('[Endpoints] visibleGroupContains - with communication BOTH', () => {
    ...
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkTrue(res, `${testName} response is an array`, Array.isArray(body));
    checkTrue(res, `${testName} contains the other user`, body.some((u: any) => u.id === teacher2.id));
    checkTrue(res, `${testName} entries have displayName field`, body.every((u: any) => typeof u.displayName === "string"));
    checkTrue(res, `${testName} entries have id field`, body.every((u: any) => typeof u.id === "string"));
  ...
  group('[Endpoints] visibleGroupContains - with communication NONE shows no users', () => {
    ...
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkTrue(res, `${testName} response is an array`, Array.isArray(body));
    checkFalse(res, `${testName} does NOT contain the other user when comm is NONE`, body.some((u: any) => u.id === teacher2.id));
...
export function testSafelyAddLinksWithUsers(data: InitData) {
  group('[Endpoints] safelyAddLinksWithUsers - sets direction to BOTH', () => {
    ...
    const body = JSON.parse(<string>res.body);
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkTrue(res, `${testName} response contains users field`, body.users !== undefined);
    checkEquals(res, `${testName} users direction is BOTH`, "BOTH", body.users);
  ...
  group('[Endpoints] safelyAddLinksWithUsers - idempotent (calling twice returns 200)', () => {
    ...
    checkEquals(res, `${testName} second call returns 200`, 200, res.status);
    checkEquals(res, `${testName} users still BOTH`, "BOTH", JSON.parse(<string>res.body).users);
...
export function testSafelyRemoveLinksWithUsers(data: InitData) {
  ...
  group('[Endpoints] safelyRemoveLinksWithUsers - returns 409 when direction cannot be changed', () => {
    ...
    checkTrue(res, `${testName} returns 200 or 409`, res.status === 200 || res.status === 409);
...
export function testAddLinkCheckOnly(data: InitData) {
  ...
  group('[Endpoints] addLinkCheckOnly - does not actually create the link', () => {
    ...
    const body = JSON.parse(<string>res.body);
    checkTrue(res, `${testName} communiqueWith still empty after check`,
  ...
  group('[Endpoints] addLinkCheckOnly - already linked groups', () => {
    ...
    checkEquals(res, `${testName} returns 200 for already-linked groups`, 200, res.status);
...
/***************************************************************************************************
 * processAddLinkAndChangeDirection
...
export function testProcessAddLinkAndChangeDirection(data: InitData) {
  group('[Endpoints] processAddLinkAndChangeDirection - both NONE: start gets INCOMING, end gets OUTGOING', () => {
    ...
    const body = JSON.parse(<string>res.body);
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkEquals(res, `${testName} start direction is INCOMING`, 'INCOMING', body[group1.id]);
    checkEquals(res, `${testName} end direction is OUTGOING`, 'OUTGOING', body[group2.id]);
    ...
    checkTrue(verifyRes, `${testName} communiqueWith now contains target`, verifyBody.communiqueWith.some((g: any) => g.id === group2.id));
  ...
  group('[Endpoints] processAddLinkAndChangeDirection - start OUTGOING: start upgrades to BOTH', () => {
    ...
    const body = JSON.parse(<string>res.body);
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkEquals(res, `${testName} start direction upgraded to BOTH`, 'BOTH', body[group1.id]);
    checkEquals(res, `${testName} end direction is OUTGOING`, 'OUTGOING', body[group2.id]);
  ...
  group('[Endpoints] processAddLinkAndChangeDirection - start INCOMING: only end changes', () => {
    ...
    const body = JSON.parse(<string>res.body);
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkTrue(res, `${testName} start not in response (no change needed)`, body[group1.id] === undefined);
    checkEquals(res, `${testName} end direction is OUTGOING`, 'OUTGOING', body[group2.id]);
  ...
  group('[Endpoints] processAddLinkAndChangeDirection - end INCOMING: end upgrades to BOTH', () => {
    ...
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkEquals(res, `${testName} start direction is INCOMING`, 'INCOMING', body[group1.id]);
    checkEquals(res, `${testName} end direction upgraded to BOTH`, 'BOTH', body[group2.id]);
  ...
  group('[Endpoints] processAddLinkAndChangeDirection - end OUTGOING: only start changes', () => {
    ...
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkEquals(res, `${testName} start direction is INCOMING`, 'INCOMING', body[group1.id]);
    checkTrue(res, `${testName} end not in response (no change needed)`, body[group2.id] === undefined);
  ...
  group('[Endpoints] processAddLinkAndChangeDirection - both already compatible: no direction change', () => {
    ...
    const verifyBody = JSON.parse(<string>verifyRes.body);
    checkTrue(verifyRes, `${testName} communiqueWith contains target`, verifyBody.communiqueWith.some((g: any) => g.id === group2.id));
...
export function testRemoveRelations(data: InitData) {
  group('[Endpoints] removeRelations - returns sender and receiver directions', () => {
    ...
    const body = JSON.parse(<string>res.body);
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkTrue(res, `${testName} response contains sender key`, 'sender' in body);
    checkTrue(res, `${testName} response contains receiver key`, 'receiver' in body);
    checkTrue(res, `${testName} sender is null or valid direction`, body.sender === null || ["NONE", "INCOMING", "OUTGOING", "BOTH"].includes(body.sender));
    checkTrue(res, `${testName} receiver is null or valid direction`, body.receiver === null || ["NONE", "INCOMING", "OUTGOING", "BOTH"].includes(body.receiver));
  ...
  group('[Endpoints] removeRelations - link disappears from outgoing/incoming', () => {
    ...
    checkFalse(resOut, `${testName} outgoing no longer contains group2`, JSON.parse(<string>resOut.body).some((g: any) => g.id === group2.id));
    ...
    checkFalse(resIn, `${testName} incoming no longer contains group1`, JSON.parse(<string>resIn.body).some((g: any) => g.id === group1.id));
...
export function testVerify(data: InitData) {
  group('[Endpoints] verify - users in same group with BOTH can communicate', () => {
    ...
    const body = JSON.parse(<string>res.body);
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkTrue(res, `${testName} response has canCommunicate field`, body.canCommunicate !== undefined);
    checkTrue(res, `${testName} canCommunicate is boolean`, typeof body.canCommunicate === "boolean");
    checkTrue(res, `${testName} canCommunicate is true`, body.canCommunicate === true);
  ...
  group('[Endpoints] verify - users in different schools cannot communicate', () => {
    ...
    const body = JSON.parse(<string>res.body);
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkEquals(res, `${testName} canCommunicate is false`, false, body.canCommunicate);
  });
  ...
  group('[Endpoints] verify - direct communication link enables verify', () => {
    ...
    checkEquals(resBefore, `${testName} cannot communicate before`, false, JSON.parse(<string>resBefore.body).canCommunicate);
    ...
    checkEquals(resAfter, `${testName} can communicate after direct link`, true, JSON.parse(<string>resAfter.body).canCommunicate);
  });
  ...
  group('[Endpoints] verify - group link enables verify', () => {
    ...
    checkEquals(res, `${testName} can communicate via group link`, true, JSON.parse(<string>res.body).canCommunicate);
...
export function testGetDiscoverVisibleUsers(data: InitData) {
  ...
  group('[Endpoints] getDiscoverVisibleUsers - with search filter', () => {
    ...
    const body = JSON.parse(<string>res.body);
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkTrue(res, `${testName} response is an empty array for non-matching search`, Array.isArray(body) && body.length === 0);
  });
  ...
  group('[Endpoints] getDiscoverVisibleUsers - empty body', () => {
    ...
    checkEquals(res, `${testName} returns 200 with empty filter`, 200, res.status);
    checkTrue(res, `${testName} response is an array`, Array.isArray(JSON.parse(<string>res.body)));
  });
}

/***************************************************************************************************
 * getDiscoverVisibleAcceptedProfile
...
export function testGetDiscoverVisibleAcceptedProfile(data: InitData) {
  group('[Endpoints] getDiscoverVisibleAcceptedProfile - returns profiles array', () => {
    ...
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkTrue(res, `${testName} response is an array`, Array.isArray(body));
    checkTrue(res, `${testName} entries are strings`, body.length === 0 || body.every((p: any) => typeof p === "string"));
  });
}

/***************************************************************************************************
 * getDiscoverVisibleStructures
...
export function testGetDiscoverVisibleStructures(data: InitData) {
  group('[Endpoints] getDiscoverVisibleStructures - returns structures list', () => {
    ...
    checkTrue(res, `${testName} response is an array`, Array.isArray(body));
    if (Array.isArray(body) && body.length > 0) {
      checkTrue(res, `${testName} entries have id field`, body.every((s: any) => typeof s.id === "string"));
      checkTrue(res, `${testName} entries have label field`, body.every((s: any) => typeof s.label === "string"));
    }
  });
}
...
export function testDiscoverVisibleAddCommuteUsers(data: InitData) {
  group('[Endpoints] discoverVisibleAddCommuteUsers - adds commute link', () => {
    ...
    const body = JSON.parse(<string>res.body);
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkTrue(res, `${testName} response contains count field`, body.count !== undefined);
    checkTrue(res, `${testName} count is a number`, typeof body.count === "number");
  });
  ...
  group('[Endpoints] discoverVisibleAddCommuteUsers - users can communicate after', () => {
    ...
    checkEquals(verifyRes, `${testName} users can communicate after add commute`, true, JSON.parse(<string>verifyRes.body).canCommunicate);
  });
}

/***************************************************************************************************
 * discoverVisibleRemoveCommuteUsers
...
export function testDiscoverVisibleRemoveCommuteUsers(data: InitData) {
  ...
  group('[Endpoints] discoverVisibleRemoveCommuteUsers - users can no longer communicate', () => {
    ...
    checkEquals(verifyRes, `${testName} users cannot communicate after remove commute`, false, JSON.parse(<string>verifyRes.body).canCommunicate);
  });
}

/***************************************************************************************************
 * discoverVisibleGetGroups
...
export function testDiscoverVisibleGetGroups(data: InitData) {
  ...
  group('[Endpoints] discoverVisibleGetGroups - created group appears in list', () => {
    ...
    if (createRes.status === 200) {
      ...
      const body = JSON.parse(<string>res.body);
      checkTrue(res, `${testName} created group appears in list`, body.some((g: any) => g.id === createdId));
    } else {
      checkTrue(createRes, `${testName} discover feature responded (may not be enabled)`, createRes.status !== 0);
    }
  });
}

/***************************************************************************************************
 * discoverVisibleGetUsersInGroup
...
export function testDiscoverVisibleGetUsersInGroup(data: InitData) {
  group('[Endpoints] discoverVisibleGetUsersInGroup - empty group returns empty array', () => {
    ...
    if (createRes.status === 200) {
      ...
      checkEquals(res, `${testName} returns 200`, 200, res.status);
      checkTrue(res, `${testName} response is an array`, Array.isArray(body));
      checkEquals(res, `${testName} group is empty`, 0, body.length);
    } else {
      checkTrue(createRes, `${testName} discover feature responded (may not be enabled)`, createRes.status !== 0);
    }
  });
  ...
  group('[Endpoints] discoverVisibleGetUsersInGroup - with users added', () => {
    ...
    if (createRes.status === 200) {
      ...
      checkEquals(res, `${testName} returns 200`, 200, res.status);
      checkTrue(res, `${testName} response is an array`, Array.isArray(body));
      checkTrue(res, `${testName} contains added user`, body.some((u: any) => u.id === teacher2.id));
    } else {
      checkTrue(createRes, `${testName} discover feature responded (may not be enabled)`, createRes.status !== 0);
    }
  });
...
export function testCreateDiscoverVisibleGroup(data: InitData) {
  group('[Endpoints] createDiscoverVisibleGroup - returns id', () => {
    ...
    const body = JSON.parse(<string>res.body);
    checkEquals(res, `${testName} returns 200`, 200, res.status);
    checkTrue(res, `${testName} response contains id`, body.id !== undefined);
    checkTrue(res, `${testName} id is a string`, typeof body.id === "string");
    checkTrue(res, `${testName} id is non-empty`, body.id.length > 0);
  });
  ...
  group('[Endpoints] createDiscoverVisibleGroup - invalid name rejected', () => {
    ...
    checkTrue(res, `${testName} empty name returns error (not 200)`, res.status !== 200);
  });
  ...
  group('[Endpoints] createDiscoverVisibleGroup - created group is retrievable', () => {
    ...
    if (createRes.status === 200) {
      ...
      checkTrue(listRes, `${testName} created group appears in groups list`, groups.some((g: any) => g.id === groupId));
    } else {
      checkTrue(createRes, `${testName} discover feature responded (may not be enabled)`, createRes.status !== 0);
    }
  });
...
export function testUpdateDiscoverVisibleGroup(data: InitData) {
  group('[Endpoints] updateDiscoverVisibleGroup - renames the group', () => {
    ...
    if (createRes.status === 200) {
      ...
      checkEquals(res, `${testName} returns 200`, 200, res.status);
      checkTrue(res, `${testName} response contains id`, body.id !== undefined);
      checkEquals(res, `${testName} id matches original`, groupId, body.id);
    } else {
      checkTrue(createRes, `${testName} discover feature responded (may not be enabled)`, createRes.status !== 0);
    }
  });
...
export function testAddDiscoverVisibleGroupUsers(data: InitData) {
  group('[Endpoints] addDiscoverVisibleGroupUsers - adds users to group', () => {
    ...
    if (createRes.status === 200) {
      ...
      checkTrue(usersRes, `${testName} user appears in group after adding`, usersBody.some((u: any) => u.id === teacher2.id));
    } else {
      checkTrue(createRes, `${testName} discover feature responded (may not be enabled)`, createRes.status !== 0);
    }
  });
  ...
  group('[Endpoints] addDiscoverVisibleGroupUsers - empty newUsers rejected', () => {
    ...
    if (createRes.status === 200) {
      ...
      checkTrue(res, `${testName} empty newUsers returns error (not 200)`, res.status !== 200);
    } else {
      checkTrue(createRes, `${testName} discover feature responded (may not be enabled)`, createRes.status !== 0);
    }
  });
  ...
  group('[Endpoints] addDiscoverVisibleGroupUsers - replaces users (old removed, new added)', () => {
    ...
    if (createRes.status === 200) {
      ...
      checkEquals(res, `${testName} replace returns 200`, 200, res.status);
      ...
      const usersBody = JSON.parse(<string>usersRes.body);
      checkTrue(usersRes, `${testName} new user is in group`, usersBody.some((u: any) => u.id === teacher3.id));
      checkFalse(usersRes, `${testName} old user is NOT in group`, usersBody.some((u: any) => u.id === teacher2.id));
    } else {
      checkTrue(createRes, `${testName} discover feature responded (may not be enabled)`, createRes.status !== 0);
    }
  });
