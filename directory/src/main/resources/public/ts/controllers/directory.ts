...
//				types:FilterOption[]		// unused
	...
	$scope.discoverVisibleSaveGroup = async function() {
		...
		if($scope.discoverVisible.displayCreateGroup){
			...
			// Track event for creation of a group
			await directory.trackEvent('DISCOVER_VISIBLE_CREATE_GROUP');
	...
	$scope.discoverVisibleCommuteUsers = async function(receiverId) {
		...
		// Track event for linking a user
		await directory.trackEvent('DISCOVER_VISIBLE_LINK_USER');
	...
	$scope.discoverVisibleRemoveCommuteUsers = async function(receiverId) {
		...
		// Track event for unlinking a user
		await directory.trackEvent('DISCOVER_VISIBLE_UNLINK_USER');
	...
	$scope.discoverVisibleExitGroup = async function() {

		// Track event for exit group
		await directory.trackEvent('DISCOVER_VISIBLE_EXIT_GROUP');
		
		await directory.discoverVisibleAddUserToGroup($scope.discoverVisible.selectedGroup, $scope.discoverVisible.displaySelectedGroupUsers.users.map(user => user.id), $scope.discoverVisible.displaySelectedGroupUsers.users.filter(user => user.id !== model.me.userId));
		await $scope.getDiscoverVisibleGetGroups();
		await $scope.backToDiscoverVisibleGroups();
	}
}]);