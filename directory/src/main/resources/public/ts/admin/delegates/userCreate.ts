...
export async function UserCreateDelegate($scope: UserCreateDelegateScope) {
  ...
  const checkField = (field: UserCreateField, method: string) => {
    if (typeof field == "undefined") {
      console.warn(
        `[Directory][UserCreate.${method}] Field should not undefined`
      );
      return false;
    }
    return true;
  ...
  const search = async (query: string) => {
    try {
      if (query && query.length >= 1 && $scope.isUserCreateSearchVisible()) {
        ...
        const filtered = founded.filter(
          (f) => (f as User).profile == "Student"
        );
  ...
  const createUserAndAttach = async () => {
    const { firstName, lastName, birthDate, type, relatives, email } =
      $scope.userCreate.form;
  ...
  // === Add listeners
  (onSearchChange as Observable<string>)
  ...
  $scope.onUserCreateSearchSelect = function () {
    if ($scope.userCreate.select && $scope.userCreate.select.innerObject)
      $scope.userCreate.form.relatives.push(
        $scope.userCreate.select.innerObject
      );
  };
  $scope.onUserCreateRemoveChild = function (user) {
    $scope.userCreate.form.relatives = $scope.userCreate.form.relatives.filter(
      (u) => u !== user
    );
  };
  ...
  const submitUserCreate = async function () {
    ...
    try {
      ...
      $scope.tracker.trackEvent(
        TRACK.event,
        TRACK.ACCOUNT_CREATION.action,
        TRACK.name(
          $scope.userCreate.addAfterSubmit
            ? TRACK.ACCOUNT_CREATION.ADD
            : TRACK.ACCOUNT_CREATION.CREATE,
          $scope.userCreate.form.type
        )
      );
  ...
  const warnNoRelatives = async function () {
    ...
    if (
      $scope.userCreate.checkRelations &&
      type === "Relative" &&
      relatives.length < 1
    ) {
  ...
  const warnOnDuplicates = async function () {
    ...
    if (type === "Student" || type === "Relative") {
      ...
      const founded = await directoryService.searchInDirectory(
        lastName,
        params
      );
      ...
      if (filtered.length) {
        ...
        await Promise.all(
          (filtered as User[]).map((u) => u.open({ withChildren: false }))
        );
  ...
  $scope.goToWarnNoRelatives = () =>
    $scope.openLightbox("admin/create-user/no-relatives");
  $scope.goToDuplicateUserForm = () =>
  ...
  $scope.attachUserToMyClass = async function () {
    ...
    if (!user) {
      console.warn(
        "[Directory][UserCreate.attachUserToMyClass] should select at least one user to attach:",
        user
      );
      return;
    }
    ...
    await directoryService.addExistingUserToClass(
      classroom.id,
      user,
      $scope.userCreate.form.relatives
    );
    afterSubmit(user);
  };
  $scope.attachUserToMyClassOnly = async function () {
    ...
    if (!user) {
      console.warn(
        "[Directory][UserCreate.attachUserToMyClassOnly] should select at least one user to attach:",
        user
      );
      return;
    }
  ...
  $scope.classnameForDuplicateUser = function (user) {
    ...
    if (classes.length === 1) {
      return lang
        .translate("classAdmin.duplicate.label.one")
        .replace("[[className]]", classes[0]);
    }
    // There are at least two classes
    return lang
      .translate("classAdmin.duplicate.label.several")
      .replace("[[classNames]]", classes.join(", "));
  };
