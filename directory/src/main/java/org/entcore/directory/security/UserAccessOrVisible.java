...
import org.entcore.common.http.filter.VisibleFilter;
import org.entcore.common.user.UserInfos;

public class UserAccessOrVisible extends UserAccess {
	...
	public void authorize(final HttpServerRequest request, Binding binding, final UserInfos user, final Handler<Boolean> handler) {
		super.authorize(request, binding, user, event -> {
			if(event){
				...
				visibleFilter.authorize(request, binding, user, handler);
			}
		});
	}
