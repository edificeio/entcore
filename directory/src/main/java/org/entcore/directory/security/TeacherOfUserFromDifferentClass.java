...
public class TeacherOfUserFromDifferentClass implements ResourcesProvider {
	...
	public void authorize(final HttpServerRequest resourceRequest, Binding binding, final UserInfos user, final Handler<Boolean> handler) {
		RequestUtils.bodyToJson(resourceRequest, event -> {
			if (event != null) {
				JsonArray userIds = event.getJsonArray("users", event.getJsonArray("ids",new JsonArray()));
				//
				String userId = resourceRequest.params().get("userId");
				if(userId!=null){
					userIds.add(userId);
				}
				//
