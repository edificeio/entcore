...
public class UserPosition {
	...
	private final String funcGroupId;

	public UserPosition(String id, String name, UserPositionSource source, String structureId, String funcGroupId) {
		this.id = id;
		this.name = name;
		this.source = source;
	...
	public static Optional<UserPosition> getUserPositionFromEncodedFunction(String dollarEncodedFunction, UserPositionSource source) {
		...
		if (functionCodification.length == 2) {
			userPosition = Optional.of(new UserPosition(null, functionCodification[1], source, functionCodification[0], null));
		} else {
			...
			if (!"ENS".equals(functionCodification[1]) 
				...
				String funcGroupId = functionCodification[0] + "$" + functionCodification[1];
				if (functionCodification[2].isEmpty()){
					userPosition = Optional.of(new UserPosition(null, functionCodification[4], source, functionCodification[0], funcGroupId));
				} else {
					userPosition = Optional.of(new UserPosition(null, functionCodification[2] + " / " + functionCodification[4], source, functionCodification[0], funcGroupId));
				}
			}
		}
		return userPosition;
