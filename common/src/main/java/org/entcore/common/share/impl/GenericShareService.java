...
public abstract class GenericShareService implements ShareService {
	...
	protected void getShareInfos(final String userId, final JsonArray actions, final JsonObject groupCheckedActions,
		...
		final FindVisibleProfileGroupsRequestDTOBuilder groupRequestBuilder = new FindVisibleProfileGroupsRequestDTOBuilder();
		final FindVisibleUsersRequestDTOBuilder userRequestBuilder = new FindVisibleUsersRequestDTOBuilder();
		final Set<String> groupIds = groupCheckedActions.fieldNames();
		final Set<String> userIds = userCheckedActions.fieldNames();
		final JsonObject groupParams = new JsonObject().put("groupIds", groupIds.stream().collect(CollectionUtils.toJsonArray()));
		groupRequestBuilder.setGroupIds(groupIds);
		final JsonObject userParams = new JsonObject().put("userIds", userIds.stream().collect(CollectionUtils.toJsonArray()));
		userRequestBuilder.setUserId(userId);
		userRequestBuilder.setUserIds(userIds);
		final String search = query.getSearch();
		if (search != null && search.trim().isEmpty()) {
			final Neo4j neo4j = Neo4j.getInstance();
			final Future<Either<String, JsonArray>> futureGroups;
			if(isReadAvailable(("getShareInfos.findGroups"))) {
				futureGroups = switchFindGroups(groupIds);
			} else {
				final Promise<Either<String, JsonArray>> promise = Promise.promise();
				neo4j.execute(GROUP_SHARED, groupParams, validResultHandler(promise::tryComplete));
				futureGroups = promise.future();
			}
			futureGroups.onSuccess(sg -> {
				...
				final Future<Either<String, JsonArray>> futureUsers;
				if(isReadAvailable(("getShareInfos.findUsers"))) {
					futureUsers = switchFindGroups(groupIds);
				} else {
					final Promise<Either<String, JsonArray>> promise = Promise.promise();
					neo4j.execute(USER_SHARED, userParams, validResultHandler(promise::tryComplete));
					futureUsers = promise.future();
				}
				futureUsers.onSuccess(event -> {
					...
					log.error("Error while getting share infos for user " + userId, th);
					handler.handle(null);
				});
			}).onFailure(th -> {
				log.error("Error while getting share infos for user " + userId, th);
			...
			if (search != null) {
				...
				groupRequestBuilder.setSearch(sanitizedSearch);
				userParams.put("search", sanitizedSearch);
				userRequestBuilder.setSearch(sanitizedSearch);
				//
			...
			if (query.getOnlyGroupsWithFilters() != null && query.getOnlyGroupsWithFilters().size() > 0) {
				...
				groupRequestBuilder.setOnlyGroupsWithFilters(new HashSet<>(query.getOnlyGroupsWithFilters()));
			...
			if (query.getOnlyUsersWithProfiles() != null && query.getOnlyUsersWithProfiles().size() > 0) {
				...
				userRequestBuilder.setOnlyUserWithProfiles(new HashSet<>(query.getOnlyUsersWithProfiles()));
			...
			if (isReadAvailable("getShareInfos.findVisibleProfileGroups")) {
				future = switchFindVisibleProfilsGroups(groupRequestBuilder.build());
			} else {
				final Promise<JsonArray> promise = Promise.promise();
				UserUtils.findVisibleProfilsGroups(eb, userId, preFilterGroup, groupQuery, groupParams, promise::tryComplete);
				future = promise.future();
			}
			future.onSuccess(visibleGroups -> {
				final JsonObject groups = new JsonObject();
				groups.put("visibles", visibleGroups);
				groups.put("checked", groupCheckedActions);
				for (Object u : visibleGroups) {
				...
				final Future<JsonArray> futureUsers;
				if (isReadAvailable("getShareInfos.findVisibleUsers")) {
					futureUsers = switchFindVisibleUsers(userRequestBuilder.build());
				} else {
					final Promise<JsonArray> promise = Promise.promise();
					UserUtils.findVisibleUsers(eb, userId, false, preFilterUser, userQuery, userParams, promise::tryComplete);
					futureUsers = promise.future();
				}
				futureUsers.onSuccess(visibleUsers -> {
					JsonObject users = new JsonObject();
					users.put("visibles", visibleUsers);
					users.put("checked", userCheckedActions);
					JsonObject share = new JsonObject().put("actions", actions).put("groups", groups)
...
//									actionsShare.addAll(new HashSet<>(actions.getList()));
	...
	private boolean isReadAvailable(String action) {
		return appMigrationConfiguration.isEnabled() && appMigrationConfiguration.getAvailableReadActions().contains(action);
	}

	private Future<Either<String, JsonArray>> switchFindGroups(Set<String> groupIds) {
		return sendToBroker("getShareInfos.findGroups", new GetGroupsRequestDTO(groupIds), GetGroupsResponseDTO.class)
			.map(response -> {
				if (response != null) {
					return new Either.Right<String, JsonArray>(response.getGroups().stream()
						.map(JsonObject::mapFrom)
						.collect(CollectionUtils.toJsonArray()));
				} else {
					return new Either.Left<String, JsonArray>("No groups found");
				}
			})
			.otherwise(th -> {
				log.error("Error while fetching groups", th);
				return new Either.Left<>("Error while fetching groups: " + th.getMessage());
			});
	}

	private Future<JsonArray> switchFindVisibleProfilsGroups(FindVisibleProfileGroupsRequestDTO request) {
		return sendToBroker("getShareInfos.findVisibleProfilsGroups", request, FindVisibleProfileGroupsResponseDTO.class)
			.map(response -> {
				if (response != null) {
	...
	private Future<JsonArray> switchFindVisibleUsers(final FindVisibleUsersRequestDTO request) {
		return sendToBroker("getShareInfos.findVisibleUsers", request, FindVisibleUsersResponseDTO.class)
			.map(response -> {
				if (response != null) {
