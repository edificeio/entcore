...
public interface PdfMetricsRecorder {
    ...
    default PdfMetricsContext onPdfGenerationStart(final TaskKind taskKind, final Content content) {
        return onPdfGenerationStart(taskKind, content, Optional.empty());
    }
    ...
    default PdfMetricsContext onPdfGenerationStart(final TaskKind taskKind, final Content content, final PdfGenerator.SourceKind sourceKind){
        return onPdfGenerationStart(taskKind, content, Optional.of(sourceKind));
    }
     ...
     * @param taskKind kind of task requiring pdf generation
     * @param content the content that need to be converted to pdf
     * @param sourceKind otpional kind of content to be converted
     * @return Context information about the generation
     */
    default PdfMetricsContext onPdfGenerationStart(final TaskKind taskKind, final Content content, final Optional<PdfGenerator.SourceKind> sourceKind){
        final PdfMetricsContext context = createContext(taskKind, content, sourceKind);
     ...
     * @return Context information about the generation
     */
    default PdfMetricsContext createContext(final TaskKind taskKind, final Content content, final Optional<PdfGenerator.SourceKind> sourceKind){
    ...
    class Content {
        ...
        public final Optional<Buffer> binary;

        private Content(String url) {
            this.url = Optional.of(url);
            this.binary = Optional.empty();
        }
