...
public class MicrometerPdfMetricsRecorder implements PdfMetricsRecorder {
    ...
    private final Gauge pendingConversion;
    ...
    public MicrometerPdfMetricsRecorder(Configuration configuration) {
        ...
        failureRate = Gauge.builder("pdf.conversion.error.rate", () -> {
            ...
            return (totalFailed+totalUnfinished) / (totalSucceed+totalFailed+totalUnfinished);
        }).description("Conversion error rate").register(registry);
        pendingConversion = Gauge.builder("pdf.conversion.pending", () -> ongoingConversions.get())
    ...
    private Counter getSucceed(final PdfMetricsContext context){
        final CounterKey key = new CounterKey(null, null, 200, context.taskKind, context.getSourceKind());
        final Counter counter = succeedConversions.computeIfAbsent(key, (k) -> {
            return Counter.builder("pdf.conversion.success")
                    ...
                    .tag("taskKind", context.taskKind.name())
                    .tag("sourceKind", context.getSourceKind())
                    .register(registry);
        });
    ...
    private Counter getFailed(final PdfMetricsContext context, final int status){
        final CounterKey key = new CounterKey(null, null, status, context.taskKind, context.getSourceKind());
    ...
    private Counter getUnfinished(final PdfMetricsContext context, final Phase phase, final String errorKind){
        final CounterKey key = new CounterKey(null, null, null, context.taskKind, context.getSourceKind());
    ...
    public void onPdfGenerationSucceed(PdfMetricsContext context) {
        ...
        succeedConversionTime.record(context.watch.elapsedTime());
    }
    ...
    public void onPdfGenerationFailed(PdfMetricsContext context, int statusCode) {
        ...
        failedConversionTime.record(context.watch.elapsedTime());
    }
    ...
    public void onPdfGenerationUnfinished(PdfMetricsContext context, Phase phase, String errorKind) {
        final Duration duration = context.watch.elapsedTime();
        ongoingConversions.addAndGet(-1);
     ...
     * The hashcode is computed from the following fields (but each of those fields could be null):
     * <ul>
     *     <li>Phase: weither the connexion has beend closed during Request or Response</li>
     *     <li>ErrorKind: the kind of error on failed</li>
     *     <li>StatusCode: HTTP status code on failed</li>
     *     <li>TaskKind: weither the counter is related to a Preview or a Print</li>
     *     <li>SourceKind: What kind of resource we are trying to convert (html, word...)</li>
     * </ul>
     */
    static class CounterKey {
        ...
        public CounterKey(Phase phase, String errorKind, Integer statusCode, TaskKind taskKind, String sourceKind) {
            this.phase = phase;
