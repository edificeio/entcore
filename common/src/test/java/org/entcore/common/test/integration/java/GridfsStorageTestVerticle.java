...
public class GridfsStorageTestVerticle extends TestVerticle {
	...
	public void start() {
		JsonObject config = new JsonObject("{\n" +
				...
				"        \"pool_size\": 10\n" +
				"      }");
		container.deployModule("io.vertx~mod-mongo-persistor~2.1.2-WSE", config, new AsyncResultHandler<String>() {
			@Override
			public void handle(AsyncResult<String> ar) {
				if (ar.succeeded()) {
					...
					storageTests = new StorageTests(new StorageFactory(vertx, new JsonObject()
							.putString("gridfs-address", "wse.gridfs.persistor")).getStorage());
