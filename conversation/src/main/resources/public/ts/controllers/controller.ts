import { $, _, Document, idiom as lang, moment, ng, notify, skin, template, ui } from 'entcore';
import { Conversation, Mail, quota, SystemFolder, User, UserFolder } from '../model';
...
export let conversationController = ng.controller('ConversationController', [
    '$scope', '$timeout', '$compile', '$sanitize', 'model', 'route', function ($scope, $timeout, $compile, $sanitize, model, route) {
        ...
        $scope.isParentOf = function (folder, targetFolder) {
            ...
            let ancestor = targetFolder.parentFolder;
            while (ancestor) {
        ...
        $scope.removeFromUserFolder = async () => {
            if (Conversation.instance.currentFolder instanceof UserFolder) {
        ...
        $scope.nextMail = async (trash?: boolean) => {
            const mails = Conversation.instance.currentFolder.mails.all;
            const idx = mails.findIndex((mail) => mail.id === $scope.state.current.id);
            let nextMail = null;
        ...
        $scope.saveDraftAuto = async () => {
            if (!$scope.draftSavingFlag) {
                ...
                const temp = $scope.state.newItem;
                setTimeout(async function () {
        ...
        $scope.refreshSignature = async (use: boolean) => {
            ...
            const signature = $scope.getSignature();
        ...
        $scope.canMarkUnread = () => {
            return Conversation.instance.currentFolder.mails.selection.selected
                    .find(e => e.getSystemFolder() !== 'INBOX') == undefined &&
        ...
        $scope.allReceivers = function (mail) {
            const receivers = mail.to.slice(0);
        ...
        $scope.filterUsers = function (mail) {
            return function (user) {
                ...
                const mapped = mail.map(user);
                return typeof mapped !== 'undefined' &&
        ...
        $scope.updateFoundUsers = async (search, model, founds, restriction?: boolean) => {
            ...
            const users = await Conversation.instance.users.findUser(search, include, exclude, restriction);
        ...
        const letterIcon = document.createElement('img');
        ...
        $scope.dropCondition = function (targetItem) {
            return function (event) {
                const dataField = event.dataTransfer.types.indexOf &&
                event.dataTransfer.types.indexOf('application/json') > -1 ?
                    'application/json' : //Chrome & Safari
                    event.dataTransfer.types.contains && event.dataTransfer.types.contains('application/json') ?
                        'application/json' : //Firefox
                        event.dataTransfer.types.contains && event.dataTransfer.types.contains('Text') ?
                            'Text' : //IE
                            undefined;
        ...
        $scope.dropTo = function (targetItem, $originalEvent) {
            ...
            const originalItems = JSON.parse($originalEvent.dataTransfer.getData(dataField));
        ...
        $scope.formatSize = function (size) {
            const formattedData = $scope.getAppropriateDataUnit(size);
        ...
        $scope.isLocalAdmin = () => model.me.functions &&
