...
import { INextcloudFolderScope } from "../../workspaceNextcloudFolder";
import { IWorkspaceNextcloudContent } from "./workspaceNextcloudContent";
import { ToolbarShareSnipletViewModel } from "./workspaceNextcloudToolbarShare";
...
export class ToolbarSnipletViewModel implements IViewModel {
  ...
  constructor(scope: IWorkspaceNextcloudContent) {
    ...
    this.copyProps = {
      ...
      onError: (e) => this.handleError(e, "Operation error"),
    };
  }
  ...
  private downloadMultipleFiles(documents: Array<SyncDocument>): void {
    ...
    // If path parent is null (meaning it's the parent folder sync), we return "/"
  ...
  public deleteDocuments(): void {
    ...
    nextcloudService
      ...
      .then(() => nextcloudService.deleteTrash(model.me.userId))
  ...
  public moveToTrash(): void {
    ...
    Promise.all(
      paths.map((path) =>
        nextcloudService.moveDocument(model.me.userId, path, `/trash/${path}`),
      ),
    )
      .then(() => nextcloudUserService.getUserInfo(model.me.userId))
      .then((userInfos) => {
      ...
      .catch((err: AxiosError) => {
        this.handleError(
          err,
          "Error while attempting to move documents to trash",
        );
        this.toggleDeleteView(false);
        this.vm.selectedDocuments = [];
        safeApply(this.vm);
  ...
  public toggleCopyView(
    ...
    if (state === true && selectedDocuments) {
  ...
  private setupCopyProps(
    ...
    this.copyProps = {
      i18: {
        ...
        actionTitle: type === "copy" ? "copy" : "move",
        actionProcessing:
      ...
      // Provide Nextcloud trees
      nextcloudTreeProvider: async () => {
        try {
          const rootFolder = new SyncDocument().initParent();
          const documents = await nextcloudService.listDocument(
            model.me.userId,
          );
          // Only include folders as potential destinations
      ...
      onCancel: () => this.closeCopyView(),
      onError: (e) => this.handleError(e, `${type} error`),
      // Handle submission based on selected folder type
      submit: (selectedFolder) => {
        if (selectedFolder instanceof models.Element) {
          // Copy/Move to workspace folder
          this.handleSubmitToWorkspace(selectedFolder, selectedDocuments, type);
        } else if (selectedFolder instanceof SyncDocument) {
          // Copy/Move to Nextcloud folder
  ...
  private async handleSubmitToWorkspace(
    ...
    try {
      this.lightbox.copy = true;

      // Extract paths of Nextcloud documents
      ...
      if (results && results.length > 0) {
        ...
        // toasts.info(`workspace.${type}.success`, { count: results.length });
  ...
  private async handleSubmitToNextcloud(
    ...
    try {
      ...
      // For move operations within Nextcloud
      if (type === "move") {
        ...
        // toasts.info(`nextcloud.move.success`, {
      ...
      // For copy operations within Nextcloud
      // Note: This might require special implementation or API
      else {
        // Implement Nextcloud copy functionality here
        // This might require download and re-upload or a specific API
        // toasts.info(`nextcloud.copy.success`, {
        //   count: sourceDocuments.length,
        // });
      }
