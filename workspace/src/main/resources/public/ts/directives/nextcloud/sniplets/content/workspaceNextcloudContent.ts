...
import { ToolbarSnipletViewModel } from "./workspaceNextcloudToolbar";
import { UploadFileSnipletViewModel } from "./workspaceNextcloudUploadFile";
import { NextcloudViewIcons } from "./workspaceNextcloudViewIcons";
import {
  INextcloudViewList,
  NextcloudViewList,
} from "./workspaceNextcloudViewList";
  ...
  [
    ...
    (
      ...
      $scope.onSelectAll = function (): void {
        $scope.documents.forEach(
          (document) => (document.selected = $scope.checkboxSelectAll),
        );
        $scope.selectedDocuments = $scope.documents.filter(
          (document: SyncDocument) => document.selected,
        );
      ...
      // Check if can drop on folder
      $scope.canDropOnFolder = function (): boolean {
        return true;
      };

      // Handle cannot drop file
