...
public class DefaultScreenTimeService implements ScreenTimeService {
    ...
    private JsonObject calculateDailyTotalScreenTime(JsonObject screenTimeData) {
        ...
        for (int i = 0; i < 24; i++) {
            // Try both zero-padded and non-zero-padded keys
            String hourKeyPadded = String.format("%02d", i);
            String hourKeyUnpadded = String.valueOf(i);
            
            JsonObject hourData = screenTimeData.getJsonObject(hourKeyPadded);
            if (hourData == null) {
                hourData = screenTimeData.getJsonObject(hourKeyUnpadded);
            }
            ...
            JsonObject durationEntry = new JsonObject()
                    .put("hour", hourKeyPadded) // Always use padded format for output
