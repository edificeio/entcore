...
import { FetchTranslationsRequestDTO, FetchTranslationsResponseDTO } from './{application}-i18n-translations-fetch.types';

@Injectable()
export class {application}I18nTranslationsFetchClient {
  ...
  async request(event: FetchTranslationsRequestDTO): Promise<FetchTranslationsResponseDTO> {
    const eventAddress = this.getSubject('{application}.i18n.translations.fetch');
