...
import { AppRegistrationRequestDTO, AppRegistrationResponseDTO } from './ent-{application}-test.types';

@Injectable()
export class Ent{application}TestClient {
  ...
  async request(event: AppRegistrationRequestDTO): Promise<AppRegistrationResponseDTO> {
    const eventAddress = this.getSubject('ent.{application}.test');
