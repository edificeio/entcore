...
import { UpsertGroupSharesRequestDTO, UpsertGroupSharesResponseDTO } from './share-group-upsert-{application}.types';

@Injectable()
export class ShareGroupUpsert{application}Client {
  ...
  async request(event: UpsertGroupSharesRequestDTO): Promise<UpsertGroupSharesResponseDTO> {
    const eventAddress = this.getSubject('share.group.upsert.{application}');
