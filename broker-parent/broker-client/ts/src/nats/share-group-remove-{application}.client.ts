...
import { RemoveGroupSharesRequestDTO, RemoveGroupSharesResponseDTO } from './share-group-remove-{application}.types';

@Injectable()
export class ShareGroupRemove{application}Client {
  ...
  async request(event: RemoveGroupSharesRequestDTO): Promise<RemoveGroupSharesResponseDTO> {
    const eventAddress = this.getSubject('share.group.remove.{application}');
