...
import { RemoveGroupSharesRequestDTO, RemoveGroupSharesResponseDTO } from './share-group-remove.types';

@Injectable()
export class ShareGroupRemoveClient {
  ...
  async request(event: RemoveGroupSharesRequestDTO): Promise<RemoveGroupSharesResponseDTO> {
    const eventAddress = this.getSubject('share.group.remove');
