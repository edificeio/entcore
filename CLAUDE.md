...
- **Constantes `static final` en tête de classe.** Elles se déclarent avec les autres champs, jamais au milieu du fichier au-dessus du bloc de méthodes qui les utilise — même quand elles ne servent qu'à une seule fonctionnalité. Un lecteur doit trouver l'état et les constantes d'une classe à un seul endroit. Pour marquer l'appartenance fonctionnelle, garder le préfixe dans le nom (`ABSENCE_*`) et une javadoc d'une ligne ; les séparateurs de section (`/* Message d'absence */`) restent réservés aux **méthodes**.
- **La validation de charge utile sort du contrôleur.** Les contrôleurs de ce repo sont déjà énormes (`ConversationController` ≈ 2 300 lignes) et grossissent à chaque feature. Une méthode de validation d'une charge utile — celle qui inspecte le corps de la requête et renvoie une clé d'erreur ou lève — va dans une classe dédiée du package `util/` du module (`AbsenceValidator.validatePayload(body)`), pas en méthode privée du contrôleur. Elle est sans état et sans dépendance : classe finale à constructeur privé, méthodes `static`, testable seule. Le contrôleur ne garde que le routage, les gardes d'autorisation et le rendu de la réponse.
- **Idem pour les utilitaires techniques.** Une méthode qui ne dépend d'aucun service injecté ni de la requête HTTP appartient au package `util/`, même si elle a besoin du `vertx` du contrôleur : il se passe en paramètre (`AsyncUtil.pause(vertx, millis)`). Le critère n'est pas « est-ce que ça touche un champ de la classe » mais « est-ce que ça relève du rôle du contrôleur ». Vérifier d'abord qu'un équivalent n'existe pas déjà dans `org.entcore:common` ou `fr.wseduc.webutils` — ces libs vivent hors de ce repo (`../edifice-entcore-libs`, `../web-utils`), donc si l'utilitaire n'y est pas, le créer dans le `util/` du module plutôt que d'ouvrir un chantier cross-repo.
