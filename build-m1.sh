...
clean () {
  docker-compose run --platform linux/amd64 --rm -u "$USER_UID:$GROUP_GID" gradle gradle clean
}
...
publish () {
  ...
  docker-compose run --platform linux/amd64 --rm -u "$USER_UID:$GROUP_GID" gradle gradle "$GRADLE_OPTION"publish
}
